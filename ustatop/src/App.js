...
export default function App() {
  ...
  const allWorkers = [...WORKERS, ...posted];
  ...
  const handleLogin = () => {
    setAuthError("");
    if (!form.phone || !form.password) return setAuthError("Barcha maydonlarni to'ldiring");
    setLoading(true);
    setTimeout(() => { setLoading(false); setUser({ name: form.phone, phone: form.phone }); setModal(null); setForm({ ...form, password: "" }); }, 1200);
  };
  ...
  const handleRegister = () => {
    ...
    if (authStep === 2 && (!form.phone || form.phone.length < 9)) return setAuthError("To'g'ri telefon kiriting");
    if (authStep === 2) return setAuthStep(3);
    if (!form.password || form.password.length < 6) return setAuthError("Parol kamida 6 ta belgi");
    setLoading(true);
    setTimeout(() => { setLoading(false); setUser({ name: form.name, phone: form.phone }); setModal(null); setAuthStep(1); }, 1200);
  };
    ...
    <div style={{ minHeight:"100vh", background:"#0D0D0D", color:"#fff", fontFamily:"'Segoe UI',system-ui,sans-serif" }}>
      ...
      {page === "profil" && user && (
        <div style={{ maxWidth:700, margin:"0 auto", padding:"60px 24px" }}>
          ...
          <button className="btn btn-ghost" style={{ width:"100%", padding:14 }} onClick={() => { setUser(null); setPage("home"); }}>
            Chiqish (Logout)
          </button>
        </div>
      )}
                ...
                {!dealDone ? (
                  <>
                    ...
                    <div style={{ textAlign:"center", padding:"16px 0 24px" }}>
                      ...
                      <textarea className="input" rows={3} placeholder="Muammoni tushuntiring... (masalan: vannaxonamda kran tomchilayapti)" style={{ marginBottom:16, resize:"none" }} />
